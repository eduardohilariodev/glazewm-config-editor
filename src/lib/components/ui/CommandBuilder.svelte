<script lang="ts">
  ...
  import type { WorkspaceConfig } from "$lib/types/config";
  import { applyMask, lengthMask, identifierMask } from "$lib/utils/masks";
...
<div class="flex flex-col gap-[0.4rem]">
  <div class="flex items-center gap-2">
    <div class="inline-flex border border-[#444] rounded overflow-hidden" role="group" aria-label="View mode">
      <button
        type="button"
        class="px-[0.6rem] py-[0.2rem] border-0 border-r border-[#444] last:border-r-0 cursor-pointer text-[0.8rem] disabled:opacity-40 disabled:cursor-not-allowed {mode === 'build' ? 'bg-[#2d4f7a] text-white' : 'bg-[#2a2a2a] text-[#ccc] enabled:hover:bg-[#3a3a3a]'}"
        disabled={!canBuild}
      ...
      <button
        type="button"
        class="px-[0.6rem] py-[0.2rem] border-0 cursor-pointer text-[0.8rem] {mode === 'raw' ? 'bg-[#2d4f7a] text-white' : 'bg-[#2a2a2a] text-[#ccc] hover:bg-[#3a3a3a]'}"
    ...
    {#if onRemove}
      <button
        type="button"
        class="ml-auto border border-[#444] bg-[#2a2a2a] text-[#f88] px-2 py-[0.2rem] rounded cursor-pointer hover:bg-[#3a3a3a]"
        onclick={onRemove}
        aria-label="Remove"
      >✕</button>
    {/if}
