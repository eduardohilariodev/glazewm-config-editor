<script lang="ts">
  import { validateRegex } from "$lib/utils/tauri";
  import {
    escapeLiteral,
    patternToTags,
    sortTags,
...
<div class="flex flex-col gap-[0.35rem] min-w-0" class:has-error={error}>
  <div class="flex items-center gap-2">
    <div class="inline-flex border border-[#444] rounded overflow-hidden" role="group" aria-label="View mode">
      <button
        type="button"
        class="px-[0.6rem] py-[0.2rem] border-0 border-r border-[#444] last:border-r-0 cursor-pointer text-[0.8rem] hover:bg-[#3a3a3a] {mode === 'tags' ? 'bg-[#2d4f7a] text-white' : 'bg-[#2a2a2a] text-[#ccc]'}"
      ...
      <button
        type="button"
        class="px-[0.6rem] py-[0.2rem] border-0 cursor-pointer text-[0.8rem] hover:bg-[#3a3a3a] {mode === 'raw' ? 'bg-[#2d4f7a] text-white' : 'bg-[#2a2a2a] text-[#ccc]'}"
    ...
    {#if !canBeTags}
      <span
        class="text-[#d8a657] text-[0.8rem] cursor-help border-b border-dotted border-[#d8a657]"
        title={decodeFailureMsg}
      >
        ⚠ can't be tagged — hover for details
      </span>
    {/if}
    {#if mode === "tags" && tags.length > 0}
      <input
        class="filter ml-auto px-2 py-[0.2rem] border border-[#444] rounded bg-[#1e1e1e] text-[#e8e8e8] font-[inherit] text-[0.8rem] min-w-[8rem] max-w-[14rem] outline-none focus:border-[#3a6aa0] transition-colors duration-[120ms] placeholder:text-[#777]"
        type="search"
        bind:value={filter}
  ...
  {#if mode === "tags"}
    <div class="chips flex flex-wrap gap-[0.35rem] items-center border rounded p-[0.3rem] bg-[#1e1e1e] min-h-[2rem] {error ? 'border-[#d83a3a]' : 'border-[#444]'}">
      {#each visibleTags as t (t)}
        ...
        <span class="inline-flex items-center gap-1 px-[0.35rem] py-[0.1rem] rounded-[3px] bg-[#2d4f7a] text-[#e8f0ff] font-mono text-[0.85rem] {tagMatches ? 'ring-1 ring-[#ffd97a]' : ''}">
          <HighlightedInput
            chip
          ...
          <button
            type="button"
            class="border-0 bg-transparent text-inherit cursor-pointer p-0 leading-none"
            aria-label="Remove"
            onclick={() => removeTag(t)}
          >✕</button>
        </span>
  ...
  {#if error}
    <p class="m-0 text-[#f88] text-[0.8rem] font-mono">⚠ {error}</p>
  {/if}
</div>
